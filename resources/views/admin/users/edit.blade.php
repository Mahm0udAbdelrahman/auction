                            ...
                            <div class="row g-4">
                                ...
                                <div class="col-md-6">
                                    ...
                                    <select class="form-select" name="country_id" id="service">
                                        <option value='null' selected selected>{{ __('Choose Country...') }}</option>
                                        @foreach ($countries as $country)
                                        <option @selected($country->id == $user->country_id) value="{{ $country->id }}">{{ $country->{'name_'.app()->getLocale()} }}</option>
                                        @endforeach
                                    </select>
                                    @error('country_id')
                                ...
                                <div class="col-md-6">
                                    ...
                                    @if ($user->image)
                                        <div class="mt-2">
                                            <img src="{{ asset($user->image) }}" alt="{{ $user->name }}" class="rounded shadow" width="100">
                                        </div>
                                    @endif
                                </div>
                                ...
                                <div class="col-md-6">
                                    ...
                                    <input type="number" name="phone" id="phone" class="form-control" placeholder="{{ __('Enter the user mobile number') }}" value="{{ old('phone', $user->phone) }}">
                                ...
                                <div class="col-md-6">
                                    ...
                                    <select class="form-select" name="service" id="service">
                                        <option value='null' selected>{{ __('Choose Service...') }}</option>
                                        <option value="vendor" {{ old('service', $user->service) === 'vendor' ? 'selected' : '' }}>{{ __('Vendor') }}</option>
                                        <option value="buyer" {{ old('service', $user->service) === 'buyer' ? 'selected' : '' }}>{{ __('Buyer') }}</option>
                                    </select>
