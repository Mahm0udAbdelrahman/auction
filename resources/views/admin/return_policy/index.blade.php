                ...
                <div class="col-12 col-xl-12">
                    <div class="card">
                        <div class="add d-flex justify-content-end p-2">
                            ...
                            <a href="{{ route('Admin.return_policy.create') }}" class="btn btn-primary"> <i
                                    class="fas fa-add"></i> {{ __('Add Return Policy') }}</a>
                            {{-- @endcan --}}
                        </div>
                        <div class="card-body">
                            <div class="table-responsive text-center">
                                <table id="example2" class="table table-striped table-bordered">
                                    ...
                                    <tbody>
                                        @forelse($data as $return_policy)
                                            <tr>
                                                <td>{{ $return_policy->id }}</td>
                                                <td>{{ __($return_policy->{'message_' . app()->getLocale()}) }}
                                                </td>
                                                <td>{{ __($return_policy->country->{'name_' . app()->getLocale()}) }}</td>

                                                <td>
                                                    {{-- @can('balance_insurances-delete') --}}
                                                    <button type="button" class="btn btn-danger delete-return_policy-btn"
                                                        data-id="{{ $return_policy->id }}">
                                                        <i class="far fa-trash-alt"></i>
                                                    </button>
                                                    {{-- @endcan --}}

                                                    {{-- @can('balance_insurances-update') --}}
                                                    <a href="{{ route('Admin.return_policy.edit', $return_policy) }}"
...
@push('scripts')
    <script>
        document.addEventListener("DOMContentLoaded", function() {
            document.querySelectorAll('.delete-return_policy-btn').forEach(button => {
                button.addEventListener('click', function() {
                    ...
                    Swal.fire({
                        ...
                        if (result.isConfirmed) {
                            ...
                            form.action = '{{ url('/admin/return_policy') }}/' + id;
                            form.method = 'POST';
