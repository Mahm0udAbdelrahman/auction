                ...
                <div class="col-12 col-xl-12">
                    <div class="card">
                        <div class="add d-flex justify-content-end p-2">
                            ...
                            <a href="{{ route('Admin.privacy_policy.create') }}" class="btn btn-primary"> <i
                                    class="fas fa-add"></i> {{ __('Add Privacy Policy') }}</a>
                            {{-- @endcan --}}
                        </div>
                        <div class="card-body">
                            <div class="table-responsive text-center">
                                <table id="example2" class="table table-striped table-bordered">
                                    ...
                                    <tbody>
                                        @forelse($data as $privacy_policy)
                                            <tr>
                                                <td>{{ $privacy_policy->id }}</td>
                                                <td>{{ __($privacy_policy->{'message_' . app()->getLocale()}) }}
                                                </td>
                                                <td>{{ __($privacy_policy->country->{'name_' . app()->getLocale()}) }}</td>

                                                <td>
                                                    {{-- @can('balance_insurances-delete') --}}
                                                    <button type="button" class="btn btn-danger delete-privacy_policy-btn"
                                                        data-id="{{ $privacy_policy->id }}">
                                                        <i class="far fa-trash-alt"></i>
                                                    </button>
                                                    {{-- @endcan --}}

                                                    {{-- @can('balance_insurances-update') --}}
                                                    <a href="{{ route('Admin.privacy_policy.edit', $privacy_policy) }}"
...
@push('scripts')
    <script>
        document.addEventListener("DOMContentLoaded", function() {
            document.querySelectorAll('.delete-privacy_policy-btn').forEach(button => {
                button.addEventListener('click', function() {
                    ...
                    Swal.fire({
                        ...
                        if (result.isConfirmed) {
                            ...
                            form.action = '{{ url('/admin/privacy_policy') }}/' + id;
                            form.method = 'POST';
